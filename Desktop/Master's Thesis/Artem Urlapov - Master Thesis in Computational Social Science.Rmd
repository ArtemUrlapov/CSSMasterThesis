...
DailyTheGuardianHeadline <- function(domain, date) {
  ...
  TheGuardianOriginalURL <- RandomTheGuardianHeadline$urlkey
  
  RegularTheGuardianURL <- gsub("com,theguardian)/", "theguardian.com/",
                                TheGuardianOriginalURL, fixed = TRUE) 
...
write.xlsx(TheGuardianDynamicDataFrame, "TheGuardianUkraine60Days.xlsx")
...
DailyHeadline <- function(domain, date) {
  ...
  Headlines <- as.data.frame(OutputList[-1, ], stringsAsFactors = FALSE)
  
  colnames(Headlines) <- OutputList[1, ]
  
  DesiredDate <- format(date, "%Y%m%d")
  
  FilteredHeadlines <- Headlines[grepl(DesiredDate, Headlines$timestamp, fixed = TRUE), ]

  RandomHeadline <- FilteredHeadlines[sample(1:nrow(FilteredHeadlines), 1), ]

  Timestamp <- RandomHeadline$timestamp
  
  TheNewYorkTimesOriginalURL <- RandomHeadline$urlkey
  
  RegularTheNewYorkTimesURL <- gsub("com,nytimes)/", "nytimes.com/",
                                    TheNewYorkTimesOriginalURL, fixed = TRUE)
  
  RetrievedTheNewYorkTimesURL <- paste0("https://web.archive.org/web/", Timestamp, "/", RegularTheNewYorkTimesURL)
  
    cat("Retrieved from The New York Times:", RetrievedTheNewYorkTimesURL, "\n")


  return(RetrievedTheNewYorkTimesURL)
}
...
TheNewYorkTimesDynamicDataFrame <- data.frame(TheNewYorkTimesDate = character(0),
                                              TheNewYorkTimesHeadline = character(0),
                                              TheNewYorkTimesWeighting = numeric(0),
                                              TheNewYorkTimesTag = character(0))

TheNewYorkTimesSplitWords <- function(string) {
...
for (i in 0:59) {
  ...
  TheNewYorkTimesTargetURL <- DailyHeadline("www.nytimes.com/international", CurrentDate)

  TheNewYorkTimesWebsite <- read_html(TheNewYorkTimesTargetURL)

  TheNewYorkTimesMaxWeighting <- 0
  
  TheNewYorkTimesMaxWeightingRow <- NULL

  for (TheNewYorkTimesTag in c("h1", "h2", "h3")) {
    TheNewYorkTimesHeadlines <- TheNewYorkTimesWebsite %>%
      html_nodes(TheNewYorkTimesTag) %>%
      html_text(trim = TRUE)


for (TheNewYorkTimesHeadline in TheNewYorkTimesHeadlines) {
    SplitHeadline <- TheNewYorkTimesSplitWords(TheNewYorkTimesHeadline)

    TheNewYorkTimesMatches1 <- sapply(SplitHeadline, function(word) any(str_detect(word, regex(paste(EnglishVectorOfWords[1:12], collapse = "|"), ignore_case = TRUE))))
    TheNewYorkTimesMatches2 <- sapply(SplitHeadline, function(word) any(str_detect(word, regex(paste(EnglishVectorOfWords[13:length(EnglishVectorOfWords)],
                                        collapse = "|"), ignore_case = TRUE))))

    if (any(TheNewYorkTimesMatches1) && any(TheNewYorkTimesMatches2)) {
        TheNewYorkTimesMatchIndex <- match(paste(SplitHeadline, collapse = " "),
                                           TheNewYorkTimesHeadlines)
        
        if (is.na(TheNewYorkTimesMatchIndex)) {
            next
        }

        TheNewYorkTimesRawWeighting <-
          length(TheNewYorkTimesHeadlines) - TheNewYorkTimesMatchIndex + 1
        TheNewYorkTimesNormalisedWeighting <- TheNewYorkTimesRawWeighting / length(TheNewYorkTimesHeadlines) 

        if (TheNewYorkTimesNormalisedWeighting > TheNewYorkTimesMaxWeighting) {
            TheNewYorkTimesMaxWeighting <- TheNewYorkTimesNormalisedWeighting
            TheNewYorkTimesMaxWeightingRow <- data.frame(TheNewYorkTimesDate =
                                                           as.character(CurrentDate), 
                                                         TheNewYorkTimesHeadline =
                                                           paste(SplitHeadline, collapse = " "),
                                                         TheNewYorkTimesWeighting = 
                                                           TheNewYorkTimesNormalisedWeighting, 
                                                         TheNewYorkTimesTag =
                                                           TheNewYorkTimesTag)
              }
        }
  ...
  if (!is.null(TheNewYorkTimesMaxWeightingRow)) {
    TheNewYorkTimesDynamicDataFrame <- rbind(TheNewYorkTimesDynamicDataFrame, TheNewYorkTimesMaxWeightingRow)
  } else {
        TheNewYorkTimesNARow <- data.frame(TheNewYorkTimesDate = as.character(CurrentDate),
                                       TheNewYorkTimesHeadline = NA,
                                       TheNewYorkTimesWeighting = 0,
                                       TheNewYorkTimesTag = NA)
    TheNewYorkTimesDynamicDataFrame <- rbind(TheNewYorkTimesDynamicDataFrame,
                                             TheNewYorkTimesNARow)
  }
}

head(TheNewYorkTimesDynamicDataFrame)
...
write.xlsx(TheNewYorkTimesDynamicDataFrame, "TheNewYorkTimesUkraine60Days.xlsx")
...
DailyElPaisHeadline <- function(domain, date) {
  ...
  ElPaisOriginalURL <- RandomElPaisHeadline$urlkey
  
  RegularElPaisURL <- gsub("com,elpais)/", "elpais.com/", ElPaisOriginalURL, fixed = TRUE)
...
write.xlsx(ElPaisDynamicDataFrame, "ElPaisUkraine60Days.xlsx")
...
DailyFolhaDeSaoPauloHeadline <- function(domain, date) {
  ...
  FolhaDeSaoPauloOriginalURL <- RandomFolhaDeSaoPauloHeadline$urlkey
  
  RegularFolhaDeSaoPauloURL <- gsub("br,com,uol,folha)/", "folha.uol.com.br/",
                                        FolhaDeSaoPauloOriginalURL, fixed = TRUE)
...
write.xlsx(FolhaDeSaoPauloDynamicDataFrame, "FolhaDeSaoPauloUkraine60Days.xlsx")
...
DailyMaverickHeadline <- function(domain, date) {
  ...
  DailyMaverickOriginalURL <- RandomDailyMaverickHeadline$urlkey
  
  RegularDailyMaverickURL <- gsub("za,co,dailymaverick)/", "dailymaverick.co.za/",
                                DailyMaverickOriginalURL, fixed = TRUE) 
...
write.xlsx(DailyMaverickDataFrame, "DailyMaverickUkraine60Days.xlsx")
...
DailyTheHinduHeadline <- function(domain, date) {
  ...
  TheHinduHeadlines <- as.data.frame(OutputList[-1, ], stringsAsFactors = FALSE)
  
  colnames(TheHinduHeadlines) <- OutputList[1, ]
  
  DesiredDate <- format(date, "%Y%m%d")
  
  FilteredTheHinduHeadlines <-
  TheHinduHeadlines[grepl(DesiredDate, TheHinduHeadlines$timestamp,
                                  fixed = TRUE), ]

  RandomTheHinduHeadline <-
    FilteredTheHinduHeadlines[sample(1:nrow(FilteredTheHinduHeadlines),
                                               1), ]

  Timestamp <- RandomTheHinduHeadline$timestamp
  
  TheHinduOriginalURL <- RandomTheHinduHeadline$urlkey
  
  RegularTheHinduURL <- gsub("com,thehindu)/", "thehindu.com/",
                                TheHinduOriginalURL, fixed = TRUE) 
  
  RetrievedTheHinduURL <- paste0("https://web.archive.org/web/", Timestamp, "/",
                                    RegularTheHinduURL)
...
TheHinduDynamicDataFrame <- data.frame(TheHinduDate = character(0),
...
for (i in 0:59) {
  ...
  CurrentDate <- StartDate + days(i)
  TheHinduTargetURL <-
    DailyTheHinduHeadline("www.thehindu.com/", CurrentDate) 

  TheHinduWebsite <- read_html(TheHinduTargetURL)

  TheHinduMaxWeighting <- 0
  TheHinduMaxWeightingRow <- NULL
...
regex(paste(EnglishVectorOfWords[1:12], collapse = "|"), ignore_case = TRUE)))
      ...
      if (EnglishVectorOfWords1 && EnglishVectorOfWords2) {
        TheHinduRawWeighting <-
          length(TheHinduHeadlines) - match(TheHinduHeadline,
                                                 TheHinduHeadlines) + 1
        TheHinduNormalisedWeighting <- TheHinduRawWeighting / length(TheHinduHeadlines) 

        if (TheHinduNormalisedWeighting > TheHinduMaxWeighting) {
          TheHinduMaxWeighting <- TheHinduNormalisedWeighting
          TheHinduMaxWeightingRow <- data.frame(TheHinduDate =
                                                    as.character(CurrentDate),
                                                   TheHinduHeadline =
                                                    TheHinduHeadline,
                                                   TheHinduWeighting = 
                                                    TheHinduNormalisedWeighting, 
  ...
  if (!is.null(TheHinduMaxWeightingRow)) {
    TheHinduDynamicDataFrame <- rbind(TheHinduDynamicDataFrame,
                                           TheHinduMaxWeightingRow)
  } else {
    TheHinduNARow <-
      data.frame(TheHinduDate = as.character(CurrentDate),
                                   TheHinduHeadline = NA,
                                   TheHinduWeighting = 0,
                                   TheHinduTag = NA)
    TheHinduDynamicDataFrame <- rbind(TheHinduDynamicDataFrame,
...
write.xlsx(TheHinduDynamicDataFrame, "TheHinduUkraine60Days.xlsx")
